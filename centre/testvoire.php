...
try {
    ...
    $sql = "SELECT  idgroupe ,nom_module21, date_seance,heure_seance, num_salle1
            FROM etudiant
            JOIN payer ON etudiant.matricul = payer.matricu5
            JOIN seance ON payer.numseance69 = seance.num_seance
            JOIN groupe ON groupe.idgroupe = seance.idgroupe14
            WHERE prenom = :prenom AND nom = :nom AND email= :email";
...
<header>
    ...
    <?php
if (!empty($resultats)) {
    echo "<h2>Bienvenue monsieur $prenom $nom, voici votre emploi du temps:</h2>";
    echo "<table border='1'>";
    echo "<tr><th>ID groupe</th><th>Module</th><th>Date de Séance</th><th>Heure de seance</th><th>Numéro de Salle</th></tr>";
    foreach ($resultats as $resultat) {
        echo "<tr>";
        echo "<td>{$resultat['idgroupe']}</td>";
        echo "<td>{$resultat['nom_module21']}</td>";
