...
<body>
    ...
    $sql_distinct_groups = "SELECT DISTINCT idgroupe10 FROM groupe_etudiant_reserve";
    ...
    if ($result_distinct_groups->num_rows > 0) {
        while ($row_group = $result_distinct_groups->fetch_assoc()) {
            $specified_groupeid = $row_group['idgroupe10'];

            $sql = "SELECT e.matricule33, e.prenom33, e.nom33,
            p.idprofesseur, p.nom_prof, p.prenom_prof,
            n.niveau_scholair, n.college_ou_lycee, n.filliere,
            m.nom_module, m.nomformation99
            FROM etudiant_reserve e, groupe_etudiant_reserve ge, groupe g, module m, niveau n, professeur p
            WHERE e.matricule33 = ge.matricule10
            AND ge.idgroupe10 = g.idgroupe
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>" . $row['matricule33'] . "</td>";
                    echo "<td>" . $row['prenom33'] . "</td>";
                    echo "<td>" . $row['nom33'] . "</td>";
        ...
        echo "Il n'y aucun groupe reserve pour le moment";
    }
